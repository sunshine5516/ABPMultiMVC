...
using ABPMultiMVC.Migrations.SeedData;
...
namespace ABPMultiMVC.Tests
{
    public abstract class ABPMultiMVCTestBase : AbpIntegratedTestBase<ABPMultiMVCTestModule>
    {
        ...
        protected void UsingDbContext(Action<ABPMultiMVCDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPMultiMVCDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPMultiMVCDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPMultiMVCDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPMultiMVCDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPMultiMVCDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPMultiMVCDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPMultiMVCDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPMultiMVCDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPMultiMVCDbContext, Task<T>> func)
        {
            T result;
